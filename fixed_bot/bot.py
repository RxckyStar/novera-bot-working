...
---------------------------------------------------------------------------
| CRITICAL WARNING: DO NOT MODIFY ASYNCIO-RELATED CODE WITHOUT READING COMPREHENSIVE_STABILITY_SOLUTION.md |
---------------------------------------------------------------------------
| The bot startup code has been carefully designed to prevent event loop errors.         |
| Our unified solution handles all known asyncio-related issues in Discord.py:           |
...
# Note: We're using safe_wait_for from discord_asyncio_fix instead of the old implementation

# Import the moderation tooltip system
try:
    import moderation_tooltip
    MODERATION_TOOLTIPS_AVAILABLE = True
except ImportError:
    logging.warning("Moderation tooltip module not available")
...
async def test_command(ctx):
    """Simple test command"""
    logging.warning(f"TEST COMMAND EXECUTED by {ctx.author.name}")
    await ctx.send(f"Test command response to {ctx.author.mention}!")
...
async def sm_command(ctx, *, message=None):
    ...
    # Debug logging
    logging.warning(f"SM COMMAND RECEIVED from {ctx.author.name} (ID: {ctx.author.id})")
    ...
    try:
        ...
        if ctx.guild:
            ...
            try:
                dm_channel = await ctx.author.create_dm()
                await dm_channel.send("Hello sweetheart~ Let's set up a message for Mommy to send! 💋")
            except discord.Forbidden:
        ...
        server_embed = discord.Embed(
            ...
            description="Which server should Mommy send this message to?",
            color=discord.Color.purple()
        )
        
        server_text = ""
        for i, (guild_id, guild_name) in enumerate(guild_list, 1):
            server_text += f"{i}. {guild_name} (ID: {guild_id})\n"
        
        server_embed.add_field(name="Available Servers", value=server_text, inline=False)
        server_embed.set_footer(text="Please enter the number of the server (e.g., '1')")
        
        await dm_channel.send(embed=server_embed)
        
        def check(m):
            return m.author == ctx.author and m.channel == dm_channel
        
        # Wait for server choice
        try:
            server_response = await safe_wait_for(bot.wait_for('message', check=check), timeout=60)
            choice = server_response.content.strip()
            
            if not choice.isdigit() or int(choice) < 1 or int(choice) > len(guild_list):
                await dm_channel.send("Oops! That's not a valid choice, darling. Let's start over~ 💕\nUse `!sm` again.")
                return
                
            guild_id, guild_name = guild_list[int(choice) - 1]
            ...
            channel_response = await safe_wait_for(bot.wait_for('message', check=check), timeout=60)
            ...
            ping_response = await safe_wait_for(bot.wait_for('message', check=check), timeout=60)
            ...
            if ping_choice == "1":
                ...
                role_response = await safe_wait_for(bot.wait_for('message', check=check), timeout=60)
                ...
                member_response = await safe_wait_for(bot.wait_for('message', check=check), timeout=60)
            ...
            message_response = await safe_wait_for(bot.wait_for('message', check=check), timeout=300)  # 5 minute timeout for longer messages
...
async def addvalue_command(ctx, *, args=""):
    ...
    try:
        ...
        # Allow bot owner
        if ctx.author.id == 859413883420016640:
            has_permission = True
        # Check for trainers role in Novera server
        elif ctx.guild and str(ctx.guild.id) == "1350165280940228629":  # Novera Team Hub
            trainer_role_id = "1350175902738419734"  # Trainers role in Novera server
...
async def getevaluated_command(ctx):
    """Request to be evaluated by a tryout evaluator in the BLR server"""
    # Channel ID for evaluation requests
    ...
    try:
        ...
        # Check which server is making the request to determine the channel
        if guild.id == 1345538548027232307:  # BLR server ID
            ...
            try:
                ...
                class EvaluationRequestButton(discord.ui.View):
                    ...
                    async def accept_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                        ...
                        # Check if the user has the evaluator role or is the owner
                        evaluator_role_id = 1350499731612110929
                        if not any(role.id == evaluator_role_id for role in evaluator.roles) and evaluator.id != OWNER_ID:
            ...
            await ctx.send("The evaluation system is only available in the BLR server, darling~ 💖")
...
async def tryouts_results_command(ctx, member: Optional[discord.Member] = None, *args):
    """Submit tryouts results for a player - evaluator role only (role ID: 1350499731612110929)"""
    ...
    try:
        ...
        # Detailed role checking with explicit logging
            ...
            evaluator_role_id = 1350499731612110929  # Specific evaluator role ID as int
            ...
            if not has_permission:
                ...
                target_names = ["evaluator", "tryouts", "tryouter", "tryout", "evaluators"]
...
async def anteup_command(ctx, amount: Optional[int] = None):
    """Ante up for a match - available to everyone"""
    # Check if this message has already been processed to avoid duplicates
    message_id = str(ctx.message.id)
    if hasattr(bot, "_processed_commands") and isinstance(bot._processed_commands, dict) and message_id in bot._processed_commands:
        logging.warning(f"Duplicate anteup command detected for message {message_id}, ignoring.")
        return
    ...
    try:
        # Make sure amount is specified
        if amount is None:
            await ctx.send("Oh sweetie, you need to specify how much you want to ante up! 💖\nUse `!anteup amount` where amount is a value in millions.")
            return
            
        # Validate amount
        if amount <= 0:
            await ctx.send("Oh darling, the amount needs to be positive! You can't ante up nothing~ 💕")
            return
        ...
        if current_value < amount:
            await ctx.send(f"Oh no sweetie, you don't have enough value to ante up {amount} million! Your current value is only {current_value} million~ 💔")
            return
        
        # Create the initial setup message and view
        setup_view = WagerSetupView(ctx, amount)
        setup_message = await ctx.author.send(
            content=f"Let's set up your wager match for {amount} million! Start by selecting a match type:", 
            view=setup_view
        )
        setup_view.message = setup_message
        
        # Let the user know to check DMs
        await ctx.send(f"{ctx.author.mention}, I've sent you a DM to set up your wager match! 💖")
        
        # Don't use the event decorator directly here as it causes conflicts
        # Instead, use buttons with callbacks in the WagerSetupView class
        logging.info(f"Anteup setup started for user {ctx.author.name} (ID: {ctx.author.id})")
        
        # Store the wager setup in a global dictionary for tracking
        if not hasattr(bot, "_wager_setups"):
            bot._wager_setups = {}
        
        # Store the wager setup information
        wager_id = f"{ctx.author.id}_{int(time.time())}"
        bot._wager_setups[wager_id] = {
        ...
        logging.info(f"Created wager setup with ID {wager_id} for user {ctx.author.name}")
        
        logging.info(f"Anteup command initiated by {ctx.author} for {amount} million")
    except discord.errors.Forbidden:
        await ctx.send(f"{ctx.author.mention}, I couldn't send you a DM. Please make sure your DMs are open and try again!")
    except Exception as e:
        logging.error(f"Error in anteup command: {e}")
        logging.error(traceback.format_exc())
        try:
            await ctx.send(random.choice(MOMMY_ERROR_VARIANTS))
        except:
...
class TeamSelectView(discord.ui.View):
    ...
    def __init__(self, user, persistent=False):
        # If persistent is True, set timeout to None for persistence
        # Otherwise use 5 minute timeout for normal usage
        super().__init__(timeout=None if persistent else 300)
        self.user = user
        self.winner = None
        self.screenshot_url = None
        
        # For persistent views, we need stable custom_ids that don't change every restart
        if persistent:
            # Create the buttons with FIXED persistent IDs
            self.team1_btn = discord.ui.Button(
                label="Team 1", 
                style=discord.ButtonStyle.primary,
                custom_id="persistent_team1_select"  # Fixed ID for persistent registration
            )
            self.team2_btn = discord.ui.Button(
                label="Team 2", 
                style=discord.ButtonStyle.primary,
                custom_id="persistent_team2_select"  # Fixed ID for persistent registration
            )
        else:
            # For non-persistent (user session) views, use unique IDs with timestamp
            timestamp = int(datetime.now().timestamp() * 1000)
            
            # Add buttons with session-specific unique IDs
            self.team1_btn = discord.ui.Button(
                label="Team 1", 
                style=discord.ButtonStyle.primary,
                custom_id=f"team1_select_{user.id}_{timestamp}"  # Dynamic ID for user session
            )
            self.team2_btn = discord.ui.Button(
                label="Team 2", 
                style=discord.ButtonStyle.primary,
                custom_id=f"team2_select_{user.id}_{timestamp}"  # Dynamic ID for user session
            )
            
        # Set up the callbacks
        self.team1_btn.callback = self.team1_callback
        self.team2_btn.callback = self.team2_callback
        
        # Add the buttons to the view
        self.add_item(self.team1_btn)
        self.add_item(self.team2_btn)
    
    async def team1_callback(self, interaction: discord.Interaction):
        """Callback for Team 1 button"""
        try:
            if interaction.user.id != self.user.id:
                await interaction.response.send_message(
                    "You can't select the winner for someone else's match!", 
                    ephemeral=True
                )
                return
            
            logging.info(f"User {interaction.user.name} selected Team 1 as winner")
            self.winner = 1
            ...
            self.team1_btn.style = discord.ButtonStyle.success
            
            # First inform the user
            await interaction.response.send_message(
                "You selected Team 1 as the winner. Please upload a screenshot showing the match results.", 
                ephemeral=True
            )
            
            # Then update the message view
            try:
                await interaction.message.edit(view=self)
                logging.info(f"Successfully updated message view after Team 1 selection")
            except Exception as edit_error:
                logging.error(f"Error editing message after Team 1 selection: {edit_error}")
                # Not critical - the selection is already recorded
                
        except Exception as e:
            logging.error(f"Error in team1_callback: {e}", exc_info=True)
            try:
                await interaction.response.send_message(
                ...
                try:
                    ...
                    logging.error("Failed to send error message for team1 button")
    
    async def team2_callback(self, interaction: discord.Interaction):
        """Callback for Team 2 button"""
        try:
            if interaction.user.id != self.user.id:
                await interaction.response.send_message(
                    "You can't select the winner for someone else's match!", 
            ...
            logging.info(f"User {interaction.user.name} selected Team 2 as winner")
            self.winner = 2
            ...
            self.team2_btn.style = discord.ButtonStyle.success
            
            # First inform the user
            await interaction.response.send_message(
                "You selected Team 2 as the winner. Please upload a screenshot showing the match results.", 
                ephemeral=True
            )
            ...
            try:
                await interaction.message.edit(view=self)
                logging.info(f"Successfully updated message view after Team 2 selection")
            except Exception as edit_error:
                logging.error(f"Error editing message after Team 2 selection: {edit_error}")
                # Not critical - the selection is already recorded
                
        except Exception as e:
            logging.error(f"Error in team2_callback: {e}", exc_info=True)
            try:
                await interaction.response.send_message(
                    "There was an error processing your selection. Please try again.",
                    ephemeral=True
                ...
                try:
                    ...
                    logging.error("Failed to send error message for team2 button")
    
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        """Ensure only the command author can interact with this view"""
        if interaction.user.id != self.user.id:
            await interaction.response.send_message(
                "This match result submission is not yours to control!", 
                ephemeral=True
            )
            return False
        return True
...
class ModVerifyView(discord.ui.View):
    """View for moderators to verify match results"""
    def __init__(self, match_info, persistent=False):
        super().__init__(timeout=None)  # No timeout for moderation
        self.match_info = match_info
        
        if persistent:
            # For persistent global registration, use fixed IDs
            self.approve_btn = discord.ui.Button(
                label="Approve Results", 
                style=discord.ButtonStyle.success,
                custom_id="persistent_approve_match"  # Fixed ID for persistent registration
            )
            self.decline_btn = discord.ui.Button(
                label="Decline Results", 
                style=discord.ButtonStyle.danger,
                custom_id="persistent_decline_match"  # Fixed ID for persistent registration
            )
        else:
            # For regular usage with specific match info, use unique IDs
            timestamp = int(datetime.now().timestamp() * 1000)
            match_id = match_info.get("submitter_id", "0") + "_" + str(timestamp)
            
            self.approve_btn = discord.ui.Button(
                label="Approve Results", 
                style=discord.ButtonStyle.success,
                custom_id=f"approve_match_{match_id}"  # Dynamic ID for specific match
            )
            self.decline_btn = discord.ui.Button(
                label="Decline Results", 
                style=discord.ButtonStyle.danger,
                custom_id=f"decline_match_{match_id}"  # Dynamic ID for specific match
            )
        
        # Set callbacks
        self.approve_btn.callback = self.approve_callback
        self.decline_btn.callback = self.decline_callback
    ...
    async def approve_callback(self, interaction: discord.Interaction):
        """Callback for approving match results"""
        try:
            # Check if user is a moderator
            if not self._is_moderator(interaction.user):
                await interaction.response.send_message(
                    "You don't have permission to verify match results.", 
                    ephemeral=True
                )
                return
            
            logging.info(f"User {interaction.user.name} approved match results from {self.match_info['submitter_name']}")
            
            # Get all participants
            try:
                winners = self.match_info["teams"][f"team{self.match_info['winner']}"]
                losers = self.match_info["teams"][f"team{3-self.match_info['winner']}"]  # 3-1=2, 3-2=1
                
                # Get the wager amount
                amount = self.match_info["amount"]
                
                # Process winners (add value)
                for winner_id in winners:
                    if winner_id:  # Make sure it's not None
                        try:
                            # Add value to winner
                            current_value = data_manager.get_member_value(winner_id)
                            new_value = current_value + amount
                            data_manager.set_member_value(winner_id, new_value)
                            logging.info(f"Added {amount} value to winner {winner_id}: {current_value} -> {new_value}")
                        except Exception as winner_error:
                            logging.error(f"Error processing winner {winner_id}: {winner_error}")
                
                # Process losers (subtract value)
                for loser_id in losers:
                    if loser_id:  # Make sure it's not None
                        try:
                            # Subtract value from loser
                            current_value = data_manager.get_member_value(loser_id)
                            new_value = max(0, current_value - amount)  # Don't go below 0
                            data_manager.set_member_value(loser_id, new_value)
                            logging.info(f"Subtracted {amount} value from loser {loser_id}: {current_value} -> {new_value}")
                        except Exception as loser_error:
                            logging.error(f"Error processing loser {loser_id}: {loser_error}")
            except Exception as processing_error:
                logging.error(f"Error processing wager match participants: {processing_error}")
                await interaction.response.send_message(
                    f"There was an error processing the participants: {processing_error}", 
                    ephemeral=True
                )
                return
            ...
            try:
                embed = discord.Embed(
                    title="✅ Match Results Approved",
                    description=f"The results for the {self.match_info['match_type']} wager match have been approved and processed.",
                    color=discord.Color.green()
                )
                
                embed.add_field(name="Winner", value=f"Team {self.match_info['winner']}", inline=True)
                embed.add_field(name="Amount", value=f"{amount} million", inline=True)
                embed.add_field(name="Approved By", value=interaction.user.display_name, inline=True)
                
                # Add screenshot if available
                if self.match_info.get("screenshot_url"):
                    embed.set_image(url=self.match_info["screenshot_url"])
                ...
                logging.info(f"Updated message with approval confirmation")
            except Exception as embed_error:
                logging.error(f"Error creating/sending approval embed: {embed_error}")
                await interaction.response.send_message(
                    "The match results were processed, but there was an error updating the message.",
                    ephemeral=True
                )
            ...
            try:
                ...
                for team_num, members in self.match_info["teams"].items():
                    for member_id in members:
                        if member_id:
                            try:
                                member = await bot.fetch_user(int(member_id))
                                if member:
                                    if team_num == f"team{self.match_info['winner']}":
                                        await member.send(
                                            f"🎉 Congratulations! Your {self.match_info['match_type']} wager match was approved, " +
                                            f"and you gained {amount} million value!"
                                        )
                                        logging.info(f"Notified winner {member_id}")
                                    else:
                                        await member.send(
                                            f"💔 Unfortunately, your {self.match_info['match_type']} wager match was lost, " +
                                            f"and you lost {amount} million value."
                                        )
                                        logging.info(f"Notified loser {member_id}")
                            except Exception as notify_error:
                                logging.error(f"Failed to notify participant {member_id}: {notify_error}")
            except Exception as notification_error:
                logging.error(f"Error in notification process: {notification_error}")
        
        except Exception as e:
            logging.error(f"Error processing wager results: {e}", exc_info=True)
            try:
                await interaction.response.send_message(
                    f"An error occurred while processing the results: {e}", 
                    ephemeral=True
                )
            except discord.errors.InteractionResponded:
                await interaction.followup.send(
    ...
    async def decline_callback(self, interaction: discord.Interaction):
        """Callback for declining match results"""
        try:
            # Check if user is a moderator
            if not self._is_moderator(interaction.user):
                await interaction.response.send_message(
            ...
            logging.info(f"User {interaction.user.name} declined match results from {self.match_info['submitter_name']}")
            
            # Create decline embed
            embed = discord.Embed(
                title="❌ Match Results Declined",
                description=f"The results for the {self.match_info['match_type']} wager match have been declined.",
                color=discord.Color.red()
            )
            
            embed.add_field(name="Declined By", value=interaction.user.display_name, inline=True)
            embed.add_field(name="Submitter", value=self.match_info["submitter_name"], inline=True)
            ...
            # Update the message
            await interaction.response.edit_message(embed=embed, view=self)
            logging.info(f"Updated message with decline confirmation")
            
            # Notify the submitter
            try:
                submitter = await bot.fetch_user(int(self.match_info["submitter_id"]))
                if submitter:
                    await submitter.send(
                        f"⚠️ Your match result submission for the {self.match_info['match_type']} wager match " +
                        f"has been declined by {interaction.user.display_name}."
                    )
                    logging.info(f"Notified submitter {self.match_info['submitter_id']}")
            except Exception as e:
                logging.error(f"Failed to notify submitter: {e}")
                
        except Exception as e:
            logging.error(f"Error declining match results: {e}", exc_info=True)
            try:
                await interaction.response.send_message(
                    f"An error occurred while declining the results: {e}", 
                    ephemeral=True
                )
            except discord.errors.InteractionResponded:
                await interaction.followup.send(
    ...
    def _is_moderator(self, user):
        ...
        # Simple check for admin, mod, or ownership
        ...
        # Owner ID hardcoded check (highest priority)
        if user.id == 654338875736588288:  # Owner ID
            return True
            
        # You could add more checks for specific roles here if needed
...
async def modhelp_command(ctx):
    ...
    try:
        ...
        await moderation_tooltip.send_moderation_tooltip(ctx)
...
async def on_message(message):
    ...
    try:
        ...
        if message.guild and not isinstance(message.channel, discord.DMChannel):
            ...
            if not is_exempt:
                ...
                bad_words = [
                    ...
                    "shit", "dammmn", "wtf", "stfu", "gtfo", "lmfao", "fml", "lmao",
                ...
                exact_check_words = [
                    "fuck", "shit", "bitch", "ass", "asshole", "damn", "hell", 
                    "crap", "bullshit", "dipshit"
                ]
                ...
                dangerous_terms = ["sybau", "kys", "kill yourself", "sy bau", "wtf", "stfu", "gtfo"]
        ...
        if not isinstance(message.channel, discord.DMChannel) and message.content:
            ...
            ultra_banned_words = ["sybau", "sybau2", "sy bau", "s y b a u"]
